%This code is intended for a lead controller
...
%locus, so we change the shape by introducing a controller zero at z_c and
%a controller pole at p_c
...
%after manual calculation, we find p_c = -10.739

controller_zero = s+2;
controller_pole = 1/(s+10.73);
lead_compensated_open_sys = uncomp_open_sys*(controller_zero*controller_pole);

%let's see the root locus now again
figure()
subplot(211), rlocus(uncomp_open_sys);
title('Uncompensated Root Locus')
subplot(212), rlocus(lead_compensated_open_sys);
...
rlocus(lead_compensated_open_sys);
sgrid(zeta, 0);
axis([-10 5 -5 5])
[k_comp, poles_comp] = rlocfind(lead_compensated_open_sys);
...
lead_compensated_closed_sys = feedback(k_comp*lead_compensated_open_sys, 1);
lead_compensated_response = step(lead_compensated_closed_sys, t);
stepinfo(lead_compensated_response,t).SettlingTime

figure()
plot(t, uncompensated_response, t, lead_compensated_response);
legend('Uncompensated response', 'Compensated response');
title('Transient improvement using Lead controller');
